...
# Copyright 2023 Anastasios Manganaris
...
token_type=char        # Transcription type (char or phn).
...
Options:
    ...
    --download_asr_model    # Download a model from Model Zoo and use it for self-supervision (default="${download_asr_model}").
...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        ...
        # [corpus dependent] Need to create data.sh for new corpus
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m "espnet2.bin.utts_train" \
                --collect_stats true \
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 8 ] && [ ${stop_stage} -ge 8 ]; then
        log "Stage 8: Evaluating: training_dir=${tts_exp}"
...
./run.sh --skip_data_prep false --skip_train true --download_asr_model ${_model_name}</code>
</pre></li>
